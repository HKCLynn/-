...
bool RMVideoCapture::set(int propId, double value)
{
    ...
    case cv::CAP_PROP_AUTO_WB:
        if (Bgain > 0 && Ggain > 0 && Rgain > 0)
            CameraSetGain(hCamera, Rgain, Ggain, Bgain);
        return true;
    case cv::CAP_PROP_XI_WB_KB:
        Bgain = value;
        return true;
    case cv::CAP_PROP_XI_WB_KG:
        Ggain = value;
        return true;
    case cv::CAP_PROP_XI_WB_KR:
        Rgain = value;
        return true;
...
bool RMVideoCapture::retrieve(cv::OutputArray image, int flag)
{
    ...
    /** @brief 现用 OpenCV 的 cvtColor 实现 -- 多线程
    cv::Mat bayerImg{cv::Size(sFrameInfo.iWidth, sFrameInfo.iHeight),
                     CV_8U,
                     pbyBuffer};
    if (channel == 1)
    {
...
bool RMVideoCapture::reconnect()
{
    ...
    sleep(1);
    open();
    set(cv::CAP_PROP_EXPOSURE, exposure_time);
    set(cv::CAP_PROP_AUTO_WB, 0);
